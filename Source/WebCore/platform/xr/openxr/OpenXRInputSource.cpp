...
{
    auto input = makeUnique<OpenXRInputSource>();
    if (XR_FAILED(input->initialize(instance, session, handeness, handle)))
        return nullptr;
    return input;
}
...
XrResult OpenXRInputSource::initialize(XrInstance instance, XrSession session, XRHandedness handeness, InputSourceHandle handle)
{
    m_instance = instance;
    m_session = session;
    ...
    String handenessName = handenessToString(handeness);
    m_subactionPathName = "/user/hand/" + handenessName;
...
XrResult OpenXRInputSource::suggestBindings(SuggestedBindings& bindings) const
{
    for (auto& profile : OpenXRInputProfiles) {
        ...
        RETURN_RESULT_IF_FAILED(createBinding(profile.path, m_gripAction, m_subactionPathName + "/input/grip/pose", bindings), m_instance);
...
XrResult OpenXRInputSource::createBinding(const char* profilePath, XrAction action, const String& bindingPath, SuggestedBindings& bindings) const
{
    ...
    auto it = bindings.find(profilePath);
    if (it != bindings.end())
...
Optional<Device::FrameData::InputSourceButton> OpenXRInputSource::getButton(OpenXRButtonType buttonType) const
{
    ...
    if (actions.touch != XR_NULL_HANDLE && XR_SUCCEEDED(getActionState(actions.touch, &result.touched)))
        hasValue = true;
    else
        result.touched = result.pressed;

    if (actions.value != XR_NULL_HANDLE && XR_SUCCEEDED(getActionState(actions.value, &result.pressedValue)))
        hasValue = true;
    else
        result.touched = result.pressed ? 1.0 : 0.0;
...
Optional<XrVector2f> OpenXRInputSource::getAxis(OpenXRAxisType axisType) const
{
    ...
    XrVector2f axis;
    if (XR_SUCCEEDED(getActionState(it->value, &axis)))
        return axis;
