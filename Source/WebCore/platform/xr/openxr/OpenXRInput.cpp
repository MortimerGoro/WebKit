...
{
    auto input = makeUnique<OpenXRInput>(instance, session, space);
...
XrResult OpenXRInput::initialize()
{
    ...
    for (auto handeness : hands) {
        m_handleIndex++;
        auto inputSource = OpenXRInputSource::create(m_instance, m_session, handeness, m_handleIndex);
        if (inputSource)
...
Vector<Device::FrameData::InputSource> OpenXRInput::getInputSources(const XrFrameState& frameState) const
{
    ...
    RETURN_IF_FAILED(xrSyncActions(m_session, &syncInfo), "xrSyncActions", m_instance, result);
